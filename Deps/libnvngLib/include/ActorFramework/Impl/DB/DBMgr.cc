...
void DBDataSaveActor::DealSave()
{
        ...
        while (true)
        {
                Call(MailResult, dbMgrActor, scDBMgrActorMailMainType, 0x2, nullptr);
                ...
                while (dbMgrActor->_saveCacheList.end() != it)
                {
                        ...
                        sqlStr.reserve(DBMgr::scSqlStrInitSize * 16);
                        std::string versionSqlStr = "END,v=CASE id ";
                        versionSqlStr.reserve(DBMgr::scSqlStrInitSize);
                        std::string inStr = "END WHERE id IN(";
                        inStr.reserve(DBMgr::scSqlStrInitSize);
                        ...
                        for (; dbMgrActor->_saveCacheList.end()!=it; ++it)
                        {
                                ...
                                if (sqlStr.size() + versionSqlStr.size() + inStr.size() > DBMgrActor::scMaxAllowedPackageSize * 0.9)
                                {
                                        ++it;
                                        break;
                                }
...
void DBDataSaveActor::DealLoad()
{
        ...
        while (true)
        {
                auto ret = Call(stDBDealList, dbMgrActor, scDBMgrActorMailMainType, 0x1, nullptr);
                if (ret->_list.empty())
                        break;
                ...
                while (ret->_list.end() != it)
                {
                        ...
                        sqlStr.reserve(DBMgr::scSqlStrInitSize);
...
void DBDataSaveActor::DealLoadVersion()
{
        ...
        while (true)
        {
                Call(stDBDealList, dbMgrActor, scDBMgrActorMailMainType, 0x0, msg);
...
// {{{ DBMgrActor
std::atomic_uint64_t DBMgrActor::scMaxAllowedPackageSize = 0;

bool DBMgrActor::Init()
{
        if (!SuperType::Init())
                return false;
...
void DBMgrActor::InitDealTimer()
{
        DealReqDBDataVersion();
        DealLoadDBData();
        DealSaveDBData();

        DBMgrActorWeakPtr weakPtr = shared_from_this();
        _timer.Start(weakPtr, 0.05, [weakPtr]() {
                auto thisPtr = weakPtr.lock();
                if (thisPtr)
                        thisPtr->InitDealTimer();
...
bool DBMgrActor::DealReqDBDataVersion()
{
...
SPECIAL_ACTOR_MAIL_HANDLE(DBMgrActor, 0x0, stDBDealList)
{
        // 先存储版本信息，再返回。
...
bool DBMgrActor::DealSaveDBData()
{
...
SPECIAL_ACTOR_MAIL_HANDLE(DBMgrActor, 0x2)
{
...
bool DBMgrActor::DealLoadDBData()
{
...
SPECIAL_ACTOR_MAIL_HANDLE(DBMgrActor, 0x1)
{
...
void DBMgrActor::InitDelVersionTimer()
{
...
void DBMgrActor::InitTerminateTimer()
{
        std::lock_guard l(_reqSaveCacheListMutex);
...
void DBMgrActor::WaitForTerminate()
{
...
bool DBMgr::Init()
{
        ...
        {
                _dbMgrActorArr[i] = std::make_shared<DBMgrActor>(i);
...
void DBMgr::Terminate()
{
        for (int64_t i=0; i<scMgrActorCnt+1; ++i)
        {
                auto act = _dbMgrActorArr[i];
                DBMgrActorWeakPtr weakPtr = act;
...
#define LOAD_DB_DATA_VERSION(sesType) \
        NET_MSG_HANDLE(sesType, E_MIMT_DB, E_MIDBST_DBDataVersion, MsgDBDataVersion) { \
                auto actor = DBMgr::GetInstance()->GetMgrActor(msg->guid()); \
                if (actor) \
                { \
                        auto info = std::make_shared<stReqDBDataVersionImpl<sesType>>(); \
         ...
         \
                        ...
                        LOG_WARN("玩家[{}] 分配 DBMgrActor 失败!!!", msg->guid()); \
                } \
        }

LOAD_DB_DATA_VERSION(DBLobbySession);
LOAD_DB_DATA_VERSION(DBLoginSession);

#define SAVE_DB_DATA(sesType) \
        NET_MSG_HANDLE(sesType, E_MIMT_DB, E_MIDBST_SaveDBData, MsgDBData, buf, bufRef) { \
                auto actor = DBMgr::GetInstance()->GetMgrActor(msg->guid()); \
                if (actor) { \
                        auto info = std::make_shared<stDBDataImpl<sesType>>(); \
                        info->_ses = shared_from_this(); \
                        info->_pb = msg; \
                        info->_bufRef = bufRef; \
                        ...
                        SendPB(msg, \
                               E_MIMT_DB, \
                               E_MIDBST_SaveDBData, \
                               sesType::MsgHeaderType::E_RMT_CallRet, \
                               msgHead._guid, \
                               msgHead._to, \
                               msgHead._from); \
                } else { \
...
SAVE_DB_DATA(DBLobbySession);
SAVE_DB_DATA(DBLoginSession);

#define LOAD_DB_DATA(sesType) \
        NET_MSG_HANDLE(sesType, E_MIMT_DB, E_MIDBST_LoadDBData, MsgDBData) { \
...
LOAD_DB_DATA(DBLobbySession);
LOAD_DB_DATA(DBLoginSession);
